...
const CustomDeck = ({ card, key }: DeckProps) => {
  ...
  const SubmitDeck = () => {
    const deck = {
      ...
      commander: "", // assuming you have this defined elsewhere
      tags: 'example,tag', // optional
      is_public: false, // or true if needed
      deck_list: selectedCards // this must be an array of { id, quantity }
    };
  
    postDeckList(deck).then((res) => {
      if (res) {
        setSelectedCards([]);
        setDeckName('');
      }
